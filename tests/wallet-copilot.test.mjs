...
test("repeat my transaction reconstructs the latest confirmed action", () => {
  const result = generateLocalAssistantResponse({
    question: "Repeat my transaction",
    messages: [],
    context: walletContext()
  });
...
test("cloud digest keeps non-sensitive metadata needed for grounded repeat reasoning", () => {
  const context = walletContext();
  context.activity.items = [
    {
      type: "Bridge",
  ...
  assert.equal(digest.activity.items[0].repeatable, true);
  assert.deepEqual(digest.activity.items[0].actionMetadata, {
    operation: "bridge",
    tokenIn: "",
    tokenOut: "",
    sourceNetwork: "arc",
