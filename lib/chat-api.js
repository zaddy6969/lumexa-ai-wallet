...
import { ARC_NETWORK_MODE, ARC_PORTFOLIO_TOKENS, ARC_APP_KIT_READY } from "./arc-chain";
import { resolveNaturalWalletAgent } from "./natural-wallet-agent";
...
export async function handleWalletChat(req, res) {
  ...
  const instantAnswer =
    resolveNaturalWalletAgent({
      question: normalizedQuestion,
      messages: normalizedMessages,
      context
    }) || getInstantWalletAnswer(normalizedQuestion, normalizedMessages, context);
  if (instantAnswer || !provider || req.body?.cloudConsent !== true) {
    return res.status(200).json(
      instantAnswer ||
        generateLocalAssistantResponse({
          question: normalizedQuestion,
  ...
  try {
    ...
    const localRecovery = getInstantWalletAnswer(normalizedQuestion, normalizedMessages, context);
    if (localRecovery) {
