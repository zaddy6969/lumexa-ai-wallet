    ...
    const loadBalances = async () => {
      ...
      try {
        ...
        if (currentChain.id === arcTestnet.id && nativeFormatted) {
          const fallbackBalance = Number(nativeFormatted);
          setAssets(
            nextAssets.map((asset) =>
              asset.symbol === "USDC"
                ? {
                    ...asset,
                    balance: formatBalanceForDisplay(nativeFormatted, "USDC"),
                    balanceValue: fallbackBalance,
                    valueUsd: asset.priceUsd ? fallbackBalance * asset.priceUsd : 0,
                    status: "ready"
                  }
                : asset
            )
        ...
        setBalanceSource("");
        setBalanceError(`Balance data is unavailable on ${currentChain.name}.`);
      } catch (error) {
        if (cancelled) return;
