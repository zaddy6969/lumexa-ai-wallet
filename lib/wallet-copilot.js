import { parsePositiveDecimal, isEvmAddress, isZeroEvmAddress } from "./wallet-validation.mjs";
import {
  ARC_APP_KIT_READY,
...
const MAINNET_ACTIONS_READY = ARC_APP_KIT_READY;
...
function formatConversation(messages) {
  return (Array.isArray(messages) ? messages : [])
    .slice(-8)
    .map((message) => {
      const content = redactContextIdentifiers(message?.content).slice(0, 1_000);
...
export function normalizePreparedWalletAction(action) {
  ...
  if (tool === "prepare_send") {
    const recipient = normalizeText(args.recipient);
    const parsed = parsePositiveDecimal(args.amount, 6);
    const amount = parsed.valid ? parsed.normalized : "";
    if (!isEvmAddress(recipient) || isZeroEvmAddress(recipient) || !amount) return null;
    return {
      id,
      kind: "wallet-action",
      tool,
  ...
  if (tool === "prepare_swap") {
    ...
    const parsed = parsePositiveDecimal(
      args.amount,
      ...
      content: [
        {
          ...
          text: [
            ...
            `User question: ${redactContextIdentifiers(question).slice(0, 800)}`
          ].join("\n\n")
        }
      ]
    }
