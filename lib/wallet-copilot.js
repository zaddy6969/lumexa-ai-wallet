...
function redactContextIdentifiers(value) {
  return redactFullTransactionHashes(value).replace(/0x[a-fA-F0-9]{40}\b/g, "[wallet address]");
}
...
function networkDisplayName(value) {
  const network = normalizeNetwork(value) || normalizeText(value).toLowerCase();
  if (network === "arc") return NETWORK_LABEL;
  if (network === "ethereum-sepolia") return "Ethereum Sepolia";
  if (network === "ethereum-mainnet") return "Ethereum";
  if (network === "base-sepolia") return "Base Sepolia";
  if (network === "base-mainnet") return "Base";
  return normalizeText(value) || "the selected network";
}
...
function networkAfterCue(text, cueSource) {
  const match = normalizeText(text).match(
    new RegExp(`\\b${cueSource}\\s+(${NETWORK_SOURCE})\\b`, "i")
  );
...
function sendRequest(question, actionPrompt, context) {
  ...
  if (!amount.amount) {
    return actionResult("How much USDC should I prepare to send?", openViewAction("send"));
  }
  if (!recipient) {
    return actionResult(
...
function bridgeRequest(question, actionPrompt, context) {
  ...
  if (!amount.amount) {
    return actionResult("How much USDC should I prepare to bridge?", openViewAction("bridge"));
  }
...
function switchRequest(actionPrompt) {
  ...
  if (!network) {
    return actionResult(`Which network should I switch to: ${NETWORK_VALUES.join(", ")}?`, null);
  }
  const action = normalizePreparedWalletAction({
    tool: "switch_network",
    args: { network }
...
function explicitOpenRequest(prompt) {
  const match = normalizeText(prompt)
    .toLowerCase()
    .match(
  ...
  const view =
    match[1] === "portfolio" ? "dashboard" : match[1] === "assistant" ? "agent" : match[1];
  return actionResult(`I can open ${view} for you.`, openViewAction(view));
}

function isRepeatLatestRequest(question) {
  const prompt = normalizeText(question).toLowerCase();
  if (!prompt) return false;
  return (
    /\b(?:same|identical)\b[\s\S]*\b(?:last|latest|previous)\b/.test(prompt) ||
    /\b(?:repeat|redo|recreate|make|prepare)\b[\s\S]*\b(?:last|latest|previous)\b/.test(prompt) ||
    /\b(?:last|latest|previous)\b[\s\S]*\b(?:again|same)\b/.test(prompt) ||
    /\b(?:repeat|redo|recreate|duplicate)\b[\s\S]*\b(?:transaction|action|transfer|send|swap|bridge)\b/.test(
      prompt
    )
  );
}

function latestActivityKind(item) {
  const kind = normalizeText(item?.kind).toLowerCase();
  const operation = normalizeText(item?.metadata?.operation).toLowerCase();
  const type = normalizeText(item?.type).toLowerCase();
  if (kind === "swap" || operation === "swap" || type.includes("swap")) return "swap";
  if (kind.includes("bridge") || operation === "bridge" || type.includes("bridge")) {
    return "bridge";
  }
  ...
  if (kind === "approval" || type.includes("approval")) return "approval";
  return kind;
}

function repeatLatestActivity(question, context) {
  if (!isRepeatLatestRequest(question)) return null;

  const latest = getLatestActivity(context);
  if (!latest) {
    return actionResult(
      "I cannot reconstruct a previous action because no recent wallet activity is loaded.",
      openViewAction("activity")
    );
  }

  const kind = latestActivityKind(latest);
  const status = normalizeText(latest.status).toLowerCase();
  if (kind === "approval") {
    return actionResult(
      "I will not duplicate a token approval from activity. Open the original record and verify the spender and allowance first.",
      openViewAction("activity")
    );
  }
  if (kind === "received") {
    return actionResult(
      "The latest item is an incoming transfer, so there is no outgoing transaction to repeat. I can open Receive or Activity instead.",
      openViewAction("activity")
    );
  }
  if (!["confirmed", "completed", "complete", "success", "successful"].includes(status)) {
    return actionResult(
      `The latest ${latest.type || "transaction"} is recorded as ${latest.status || "unconfirmed"}. I will not prepare a duplicate until the original is confirmed.`,
      openViewAction("activity")
    );
  }

  if (kind === "sent") {
    const amount = amountDetails(latest.amount, 6).amount;
    const recipient = extractAddress(
      latest.recipient || latest.counterparty || latest.receiver || ""
    );
    const action = normalizePreparedWalletAction({
      tool: "prepare_send",
    ...
    if (!action) {
      return actionResult(
        "I found the latest transfer, but its full recipient or amount is not available in the loaded record. Open Send and enter those details again.",
        openViewAction("send")
      );
    }
    return actionResult(
      `I reconstructed your latest confirmed transfer and prepared ${amount} USDC to ${shortAddress(recipient)} for review. Nothing has been submitted—verify the full address, current balance and live fee before signing.`,
      action
    );
  }
  ...
  if (kind === "swap") {
    ...
    const amount = amountDetails(latest.amount, 8).amount;
    const action = normalizePreparedWalletAction({
      tool: "prepare_swap",
      args: {
    ...
    if (!action) {
      return actionResult(
        "I found the latest swap, but its exact pair or amount is missing from the loaded record. Open Swap to review and enter the current details.",
        openViewAction("swap")
      );
    }
    return actionResult(
      `I reconstructed your latest confirmed swap and prepared ${action.args.amount} ${action.args.tokenIn} → ${action.args.tokenOut} for review. Nothing has been submitted—fetch a fresh quote and verify slippage before signing.`,
      action
    );
  }
  ...
  if (kind === "bridge") {
    ...
    const action = normalizePreparedWalletAction({
      tool: "prepare_bridge",
      args: {
        sourceNetwork: metadata.sourceNetwork,
        destinationNetwork: metadata.destinationNetwork,
        amount
      }
    });
    if (!action) {
      return actionResult(
        "I found the latest bridge, but its exact route or amount is missing from the loaded record. Open Bridge to choose the route again.",
        openViewAction("bridge")
      );
    }
    return actionResult(
      `I reconstructed your latest confirmed bridge and prepared ${action.args.amount} USDC from ${networkDisplayName(action.args.sourceNetwork)} to ${networkDisplayName(action.args.destinationNetwork)} for review. Nothing has been submitted—verify the live route, balance, fees and required signatures.`,
      action
    );
  }

  return actionResult(
    "The latest activity is not a supported send, swap, or bridge that Lumexa can safely reconstruct.",
    openViewAction("activity")
  );
}

export function resolveRepeatLatestAction(context) {
  return repeatLatestActivity("Repeat my latest transaction", context);
}
...
function resolveActionRequest(question, messages, context) {
  ...
  if (educationalAction(question))
    return walletEducation(question) || walletEducation(actionPrompt);
...
function reviewSummary(context) {
  ...
  parts.push(
    "If you do not recognize an approval or transfer, verify it in the explorer before acting."
  );
  return parts.join(" ");
}
...
function explainActivity(question, context) {
  ...
  const hash = normalizeText(question)
    ...
    ?.toLowerCase();
  const shortHash = normalizeText(question).match(
    /0x[a-fA-F0-9]{6,16}(?:…|\.\.\.)[a-fA-F0-9]{4,12}/
  )?.[0];
  const target = hash
    ? items.find((item) => normalizeText(item?.txHash).toLowerCase() === hash)
    : shortHash
      ? items.find(
          (item) => normalizeText(item?.txHashShort).toLowerCase() === shortHash.toLowerCase()
        )
      : items[0];
...
function capabilitiesSummary() {
  ...
  return [
    "I can help with:",
    ...transactionTools.map((tool) => `• ${tool}`),
    "Ask naturally—for example, “Send 5 USDC to 0x…”, “Swap 10 USDC to EURC”, or “Explain my latest transaction.”",
    "Every transaction remains review-only until you sign it."
  ].join("\n");
}
  ...
  return {
    ...
    activity: {
      ...
      items: items.map((item) => {
        ...
        const kind = latestActivityKind(item);
        ...
        return {
          ...
          chain: redactContextIdentifiers(item.chain).slice(0, 100),
          ...
          repeatable:
            ["sent", "swap", "bridge"].includes(kind) &&
            ["confirmed", "completed", "complete", "success", "successful"].includes(status),
          actionMetadata: {
            operation: normalizeText(metadata.operation).slice(0, 24),
            tokenIn: normalizeToken(metadata.tokenIn),
            tokenOut: normalizeToken(metadata.tokenOut),
            sourceNetwork: normalizeNetwork(metadata.sourceNetwork),
            destinationNetwork: normalizeNetwork(metadata.destinationNetwork),
            slippageBps: [50, 100, 300].includes(Number(metadata.slippageBps))
              ? Number(metadata.slippageBps)
              : null
          }
        };
      })
...
export function generateLocalAssistantResponse({ question, messages, context }) {
  const actionResponse =
    repeatLatestActivity(question, context) || resolveActionRequest(question, messages, context);
  let answer;
  let actions = [];
  ...
  if (actionResponse) {
    ...
    if (intent === "secret_safety") {
      ...
      answer =
        "Hi — I’m Lumexa, your wallet copilot. I can read the balances and activity visible here, explain what happened, and prepare safe actions for your review. What would you like to do?";
